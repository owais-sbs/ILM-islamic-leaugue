...
export function ReviewQueue({
  ...
  const reviewItems = articles.filter((a) => a.status === 'submitted' || a.status === 'approved' || a.status === 'returned');
            ...
            {reviewItems.map((article) => (
              <tr key={article.id} className="border-b border-ilm-navy/5 last:border-0 align-top">
                <td className="px-5 py-4">
                  ...
                  <p className="mt-0.5 text-xs text-ilm-navy/40">{article.category} · {article.date}</p>
                  {article.reviewNotes && (
                  ...
                  {notesFor === article.id && (
                    <div className="mt-3 space-y-2">
                      <textarea
                        ...
                        placeholder="Review notes are required to return…"
                        rows={3}
                ...
                <td className="px-5 py-4">
                  <div className="flex flex-wrap justify-end gap-2">
                    <button onClick={() => onPreview(article)} className="inline-flex items-center gap-1 rounded-full border border-ilm-navy/10 px-3 py-1.5 text-[11px] font-bold uppercase text-ilm-navy">
                      <Eye size={13} /> Preview
                    </button>
                    <button onClick={() => onEdit(article)} className="inline-flex items-center gap-1 rounded-full border border-ilm-navy/10 px-3 py-1.5 text-[11px] font-bold uppercase text-ilm-navy">
                      <Pencil size={13} /> Edit
                    </button>
                    {canApprove(role) && article.status === 'submitted' && (
                      <button onClick={() => onApprove(article)} className="inline-flex items-center gap-1 rounded-full bg-green-600 px-3 py-1.5 text-[11px] font-bold uppercase text-white">
                    ...
                    {canPublish(role) && article.status === 'approved' && (
                      <button onClick={() => onPublish(article)} className="inline-flex items-center gap-1 rounded-full bg-ilm-gold px-3 py-1.5 text-[11px] font-bold uppercase text-ilm-navy-deep">
                        <Upload size={13} /> Publish
                      </button>
                    )}
                    {article.status === 'approved' && !canPublish(role) && (
