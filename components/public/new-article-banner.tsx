...
export function NewArticleBanner() {
  const { publishedArticles, siteSettings } = useIlm();
  ...
  useEffect(() => {
    try {
      const key = `ilm-announce-${siteSettings.announcementBanner}`;
      setAnnounceDismissed(sessionStorage.getItem(key) === '1');
    } catch {
  ...
  const announcement = siteSettings.announcementBanner?.trim();
    ...
    <>
      <AnimatePresence>
        {announcement && !announceDismissed && (
          ...
          >
            <div className="border-b border-ilm-navy/10 bg-ilm-navy text-white">
              <div className="relative mx-auto flex max-w-[1280px] items-center gap-3 px-4 py-2.5 sm:px-6 lg:px-8">
                <p className="min-w-0 flex-1 text-[12.5px] leading-snug sm:text-[13px]">{announcement}</p>
                <button
                  type="button"
                  ...
                  onClick={() => {
                    ...
                    try {
                      sessionStorage.setItem(`ilm-announce-${announcement}`, '1');
                    } catch {
                      /* ignore */
                    }
                  }}
