...
export function ArticlesLibraryClient({
  articles,
  categories,
  authors,
}: {
      ...
      <div className="border-b border-slate-100 bg-white px-6 py-4 md:px-12">
        <div className="mx-auto max-w-7xl space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            <span className="w-14 shrink-0 text-[9px] font-semibold uppercase tracking-[.2em] text-slate-400">
              Subject
            </span>
            <button
              type="button"
              ...
              className={cn(
                'rounded-full px-3 py-1.5 text-[10.5px] font-medium transition',
                !activeCategory
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {activeAuthors.map((a) => (
              <button
                ...
                className={cn(
                  'flex items-center gap-1.5 rounded-full px-2.5 py-1.5 text-[10.5px] font-medium transition',
                  activeAuthor === a.id
                    ? 'bg-ilm-gold text-white'
                    : 'border border-slate-200 bg-white text-slate-600 hover:border-ilm-gold hover:text-ilm-gold',
                )}
              >
                {a.avatar ? (
                  ...
                  <span className="flex h-4 w-4 items-center justify-center rounded-full bg-slate-200 text-[8px]">
                    {a.name.charAt(0)}
                  </span>
                )}
            ...
            <div className="flex items-center gap-2 pt-0.5">
              <p className="text-[11px] text-slate-400">
                ...
                {activeCategory && (
                  <>
                    {' '}
                    in <strong className="text-ilm-navy">{activeCategory}</strong>
                  </>
                )}
                {activeAuthor && (
                  <>
                    {' '}
                    by{' '}
                    <strong className="text-ilm-navy">
              ...
              <button
                type="button"
                onClick={clearAll}
                className="text-[10px] font-semibold text-ilm-gold underline"
              >
                Clear all
              </button>
            </div>
                  ...
                  {categories.map((cat, i) => (
                    <Reveal key={cat.id} delay={`delay-${(i % 3) + 1}`}>
                      ...
                      >
                        ...
                        <ArrowUpRight
                          size={16}
                          className="text-ilm-gold/40 transition group-hover:text-ilm-gold"
                        />
                      </button>
                    </Reveal>
                  ))}
